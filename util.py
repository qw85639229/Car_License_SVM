...
if __name__ == '__main__':
    ...
    # print(img.shape)
    ratio = 0.1
    img = cv2.resize(img, (int(img.shape[1] / ratio), int(img.shape[0] / ratio)), cv2.INTER_NEAREST)
    ...
    thresh = cv2.resize(thresh, (int(img.shape[1]/0.5), int(img.shape[0]/0.5)), cv2.INTER_NEAREST)
    print(type(thresh))
    print(thresh.shape)
    time_end = time.time()
